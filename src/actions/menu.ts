...
// ----------------------------------------------------
// SEED SEED DATA IF DATABASE IS EMPTY
// ----------------------------------------------------

export async function seedInitialMenuData() {
  try {
    const categoryCount = await prisma.category.count()
    if (categoryCount > 0) return { success: true, seeded: false }

    // Create Categories
    const burgersCat = await prisma.category.create({
      data: {
        name: 'Gourmet Burgers',
        description: 'Juicy handcrafted beef & chicken burgers made fresh to order',
      },
    })
    ...
    const pizzaCat = await prisma.category.create({
      data: {
        name: 'Artisan Pizzas',
        description: 'Wood-fired sourdough pizzas with premium toppings',
      },
    })
    ...
    const beverageCat = await prisma.category.create({
      data: {
        ...
        description: 'Chilled sodas, fresh juices, and specialty coffees',
      },
    })
    ...
    const sidesCat = await prisma.category.create({
      data: {
        name: 'Sides & Snacks',
        description: 'Crispy fries, mozzarella sticks, and loaded bites',
      },
    })
    ...
    const burger1 = await prisma.menuItem.create({
      data: {
        ...
        description: 'Double Angus beef patty, sharp cheddar cheese, crispy bacon, smokey BBQ sauce.',
        basePrice: 1250,
        isAvailable: true,
    ...
    const burger2 = await prisma.menuItem.create({
      data: {
        name: 'Crispy Zinger Supreme',
        description: 'Spicy fried chicken fillet, jalapenos, melted cheese slice, tangy mayo.',
        basePrice: 950,
        isAvailable: true,
        categoryId: burgersCat.id,
      },
    ...
    const pizza1 = await prisma.menuItem.create({
      data: {
        ...
        description: 'Wild mushrooms, mozzarella, truffle cream sauce, fresh basil.',
        basePrice: 1850,
        isAvailable: true,
    ...
    await prisma.menuItem.create({
      data: {
        ...
        description: 'Seasoned crinkle-cut fries topped with cheese sauce & peri peri seasoning.',
        basePrice: 450,
    ...
    await prisma.menuItem.create({
      data: {
        ...
        description: 'Espresso with cold milk, vanilla syrup, and rich caramel drizzle.',
        basePrice: 520,
        isAvailable: true,
    ...
    // Create Add-Ons & Modifiers
    await prisma.addOn.createMany({
      data: [
        { name: 'Extra Cheddar Cheese Slice', price: 120, isAvailable: true, menuItemId: burger1.id },
        { name: 'Double Beef Patty', price: 350, isAvailable: true, menuItemId: burger1.id },
        { name: 'Extra Garlic Dip Sauce', price: 80, isAvailable: true, menuItemId: null },
        { name: 'Spicy Jalapenos Extra', price: 60, isAvailable: true, menuItemId: burger2.id },
        { name: 'Extra Mozzarella Cheese', price: 200, isAvailable: true, menuItemId: pizza1.id },
      ],
    })
    ...
    return { success: false, error: 'Failed to seed menu data' }
  }
}
